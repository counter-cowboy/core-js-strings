...
function countVowels(str) {
  return str.replace(/[^eyuioa]/gi, '').length;

  // let count = 0;
  // const vowels = ['a', 'e', 'y', 'u', 'i', 'o'];
  // for (let i = 0; i < str.length; i += 1)
  //   if (vowels.includes(str.toLowerCase()[i])) count += 1;
  // return count;
}
...
function findLongestWord(sentence) {
  ...
  let theLongestWord = sentArray[0];
  let theLongest = theLongestWord.length;
  for (let i = 0; i < sentArray.length; i += 1) {
    if (sentArray[i].length > theLongest) {
      theLongestWord = sentArray[i];
      theLongest = sentArray[i].length;
    }
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < strArray.length; i += 1) {
    ...
    else strArray[i] = strArray[i].toLowerCase();
  }
...
function encodeToRot13(str) {
  ...
  return strArr.map((letter) => rot13[alphabet.indexOf(letter)]).join('');
}
...
function getCardId(value) {
  ...
  return (
    card.indexOf(value[0]) + suit.indexOf(value[value.length - 1]) * card.length
  );
}
